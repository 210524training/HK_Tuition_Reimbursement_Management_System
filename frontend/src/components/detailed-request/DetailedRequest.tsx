...
const DetailedRequest: React.FC<Props> = ({request, currentUser}) => {
  ...
  const handleOnApproveOrReject = (id: string | undefined, status: undefined | string, request: Reimbursement | undefined) => {
  ...
  const handleOnReturn = (docid: string | undefined, sendTo: ReRoute | undefined, comments: string) => {
    ...
    // send up the chain
    sendStatusUpdate(docid, newStatus);
    history.goBack()
  }
  ...
  const handleUpdateGrade = async (grade: string, request: Reimbursement | undefined) => {
    // call axios
    await sendUpdateFinalGrade(grade, request?.id)
    history.goBack()
  }
  ...
  const handleChangeReimbursement = async (e: FormEvent<HTMLFormElement>) => {
    ...
    sendUpdateAmount(request?.id, amt, isExceedingFunds());
    history.goBack();
  }
...

        {
          ...
          (<>
            <div className="mt-2 container">
              ...
              <Button variant="dark" 
                size="lg" 
                onClick={() => handleOnApproveOrReject(request?.id, request?.status, request)}>
                Approve
              </Button>{' '}
        ...
        }
        {
          ...
          <div className="mt-4 container">
            ...
            <Button variant="danger" 
              className="mt-4"
              size="lg" 
              onClick={() => handleOnApproveOrReject(request?.id, 'Reject', request)}>
                Reject
            </Button>{' '}
          </div>
          </> )
      ...
      <div className="mt-4 container">
        ...
        <div className="ml-3">
          <Button 
            className="mb-2" 
            variant="dark" 
            onClick={() => handleOnReturn(request?.id, 'Send to Employee', returnMessage)}>
            Employee
          </Button> {' '}
        </div>
        ...
        <div className="ml-3">
          <Button 
            className="mb-2" 
            variant="dark" 
            onClick={() => handleOnReturn(request?.id, 'Send to Direct Supervisor', returnMessage)}>
            Direct Supervisor
          </Button> {' '}
        </div>
        ...
        <div className="ml-3">
          <Button 
            className="mb-2" 
            variant="dark" 
            onClick={() => handleOnReturn(request?.id, 'Send to Department Head', returnMessage)}>
            Department Head
          </Button> {' '}
        </div>
        ...
        <div className="ml-3">
          <Button 
            className="mb-2" 
            variant="dark" 
            onClick={() => handleOnReturn(request?.id, 'Send to Benefits Coordinator', returnMessage)}>
